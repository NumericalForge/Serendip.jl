...
function _mesh_json_data(mesh::AbstractDomain)
    nodes_obj = [ node.coord for node in mesh.nodes ]

    cells_obj = [
        OrderedDict(
            "shape" => cell.shape.kind,
    ...
    node_data_obj = Dict{String,Array}( k => V for (k,V) in mesh.node_fields if size(V,2)==1 )
    elem_data_obj = Dict{String,Array}( k => V for (k,V) in mesh.elem_fields if size(V,2)==1 )

    return OrderedDict(
        "nnodes"         => length(mesh.nodes),
        "nelems"         => length(mesh.elems),
        "nodes"          => nodes_obj,
        "elements"       => cells_obj,
        "faces"          => faces_obj,
...
"""
    mesh_json(mesh; pretty=true) -> String

Return a JSON representation of a mesh. The same representation is written by
`save(mesh, filename)` when `filename` has a `.json` extension.
"""
function mesh_json(mesh::AbstractDomain; pretty=true)
    add_extra_fields(mesh)
    data = _mesh_json_data(mesh)
    return JSON.json(data, pretty=pretty)
end
...
Saves a mesh object into a file. Available formats are vtk, vtu and json.
"""
function save(mesh::AbstractDomain, filename::String; compress=true, quiet=false)
    ...
    if format==".json"
        data = _mesh_json_data(mesh)
        write(filename, JSON.json(data, pretty=true))
    else
