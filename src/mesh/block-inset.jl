...
function Base.copy(bl::BlockInset; dx=0.0, dy=0.0, dz=0.0)
    BlockInset(get_coords(bl.points) .+ [dx dy dz], curvetype=bl.curvetype, closed=bl.closed,
                       embedded=bl.embedded, shape=bl.cellshape, tag=bl.tag,
...
function split_block!(mesh::Mesh, bl::BlockInset)
    ...
    if bl.tips in (:front, :both)
        joint = newjoints[1]
        tip = joint.couplings[2].nodes[1]
        tipjointpts  = vcat(joint.couplings[1].nodes, tip)
        tipjointcell = Cell(TIPJOINT, tipjointpts, tag=bl.tiptag)
        tipjointcell.couplings = joint.couplings
        push!(mesh.elems, tipjointcell)
    end
    if bl.tips in (:end, :both)
        joint = newjoints[end]
        tip = joint.couplings[2].nodes[2]
        tipjointpts  = vcat(joint.couplings[1].nodes, tip )
        tipjointcell = Cell(TIPJOINT, tipjointpts, tag=bl.tiptag)
        tipjointcell.couplings = joint.couplings
        push!(mesh.elems, tipjointcell)
