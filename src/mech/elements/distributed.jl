...
function mech_boundary_forces(elem::Element, facet::Cell, t::Float64, key::Symbol, val::Union{Real,Symbol,Expr,Symbolic})
    ...
    isedgebc && ctx.stress_state==:axisymmetric && error("mech_boundary_forces: boundary condition $key is not applicable in a axisymmetric analysis")
    ...
    # generate a map
    keys = (:ux, :uy, :uz)[1:ndim]
    map  = [ get_dof(node, key).eq_id for node in facet.nodes for key in keys ]
