...
compat_elem_types(::Type{DamageConcrete}) = (MechBulk,)
...
function calcD(mat::DamageConcrete, state::DamageConcreteState)
    ...
    D  = calcDe(E, ν, state.ctx.stress_state)
    return D
end


#function calcDsec(mat::DamageConcrete, state::DamageConcreteState, Δε::Array{Float64,1}, stress_state::Symbol)
...
function state_values(mat::DamageConcrete, state::DamageConcreteState)
    dict = stress_strain_dict(state.σ, state.ε, state.ctx.stress_state)
