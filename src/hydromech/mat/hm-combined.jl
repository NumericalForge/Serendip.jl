...
compat_state_type(::Type{HMCombined{M1,M2}}, ::Type{HMSolid}) where {M1,M2} = HMCombinedState{compat_state_type(M1,SeepSolid,ctx), compat_state_type(M2,MechSolid,ctx)}
compat_state_type(::Type{HMCombined{M1,M2}}, ::Type{HMJoint}) where {M1,M2} = HMCombinedState{compat_state_type(M1,HydroJoint,ctx), compat_state_type(M2,MechContact,ctx)}
...
function update_state(mat::HMCombined, state::HMCombinedState, Δε::Vector{Float64}, Δut::Float64, G::Vector{Float64}, Δt::Float64)
