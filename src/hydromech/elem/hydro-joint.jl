...
compat_role(::Type{HydroJoint}) = JOINTCELL
...
function elem_conductivity_matrix(elem::HydroJoint)
    ...
    for ip in elem.ips
        ...
        coef = detJ*ip.w*th*(elem.pmodel.w^3)/(12*elem.pmodel.η)
        H -= coef*Bf'*Bf
    end
...
function elem_compressibility_matrix(elem::HydroJoint)
    ...
    for ip in elem.ips
        ...
        # compute Cpp
        coef = detJ*ip.w*elem.pmodel.β*elem.pmodel.w*th
        Cpp -= coef*Nf'*Nf
    end
...
function elem_internal_forces(elem::HydroJoint, F::Array{Float64,1})
    ...
    th       = elem.ctx.thickness
    nnodes   = length(elem.nodes)
    bsnodes  = elem.shape.base_shape.npoints
    ...
    for ip in elem.ips
        # compute shape Jacobian
        N    = fshape.func(ip.R)
        dNdR = elem.shape.base_shape.deriv(ip.R)
...
function update_elem!(elem::HydroJoint, U::Array{Float64,1}, Δt::Float64)
    ...
    for ip in elem.ips
        ...
        Vt, L = update_state(elem.pmodel, ip.state, Δuw, G, BfUw, Δt)
