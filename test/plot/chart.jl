...
add_series(chart, X, Y1, mark=:circle, line_width=0.5, label=L"\sin(x)")
add_series(chart, X, Y2, mark=:utriangle, color=:royalblue, label=L"\cos(x)")
add_series(chart, X, Y3, mark=:square, color=:green, line_style=:dash, label=L"\sin(x)\, \cos(x)")
